...
public class FacePamphlet extends Program implements FacePamphletConstants {
	...
	private JButton LookupButton, DeleteButton, AddButton, changestatusButton, changepictureButton, addfriendButton, birthbutton;
	private JTextField nametextfield, changestatustextfield, changepicturetextfield, addfriendtextfield, birthtext;
	...
	public void init() {
		...
		birthtext = new JTextField(TEXT_FIELD_SIZE);
		birthbutton = new JButton("Add birthday");
		
		birthtext.addKeyListener(new MyKeyListener(birthtext));
		...
		addActionListeners();
		canvas = new FacePamphletCanvas(profileBasa); 
		add(canvas);
		...
		public void keyPressed(KeyEvent arg0) {
			...
			if (arg0.getKeyCode() == KeyEvent.VK_ENTER) {
				if (source.equals(changestatustextfield)) {
					...
					addfriend();
				}else if(source.equals(birthtext)) {
					addbirthday();
				}
	...
	private void lookup() {
		...
		System.out.println(profileBasa.toString());
	}
	...
	private void addfriend() {
		...
		if (name != EMPTY_LABEL_TEXT && profileBasa.containsProfile(mininame)) {
			if (profile.addFriend(name)) {
				...
				FacePamphletProfile tt = profileBasa.getProfile(name);
				if(tt!=null)
				tt.addFriend(mininame);
	...
	private void addbirthday() {
		...
		System.out.println("asjdb");
		if (birthday != EMPTY_LABEL_TEXT && profileBasa.containsProfile(mininame)) {
			profile.setbirthday(birthday);
			canvas.displayProfile(profile);
			canvas.showMessage("Birthday updated to " + birthday);
		}else {
			canvas.showMessage("Please select a profile to change birthday");
		}
	}
