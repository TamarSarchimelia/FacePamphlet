...
public class FacePamphletCanvas extends GCanvas implements FacePamphletConstants, ComponentListener {
	...
	public FacePamphletCanvas(FacePamphletDatabase _profileBasa) {
		profileBasa=_profileBasa;
		// You fill this in
		label = new GLabel("");
		name = new GLabel("");
		rect = new GRect(0, 0);
...
// here is code for message 
	...
	public void displayProfile(FacePamphletProfile profile) {
		...
		if(profile.getbirthday()!=EMPTY_LABEL_TEXT) {
			
			birth.setLabel(profile.getbirthday());
			birth.setFont(PROFILE_STATUS_FONT);
			birth.setLocation(LEFT_MARGIN, y + status.getHeight());
			add(birth);
		}
	...
	private void friend(FacePamphletProfile profile) {
		...
		list = new GLabel(" ");
		    ...
		    String t = "";
		    FacePamphletProfile pp=profileBasa.getProfile(s);
		    if (pp!=null) t+=pp.toString();
			list.setLabel(t);
